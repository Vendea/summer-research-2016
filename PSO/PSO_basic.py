...
class PSO:
	...
	def __init__(self, comm, cost, feed, sess, b_up, b_lo, omega, phi_p, phi_g):
		...
		self.vel = [random.randrange(b_lo-b_up, b_up-b_lo) for _ in range(np.size(self.var))]

	def optimize(self):
		pos = 0
		var_new = []
		for sa,v,ph,pb,gb in zip(self.single_assign, self.var, self.pl, self.my_best, self.g_best):
			v_n = []
	...
	def update_group_best(self):
		...
		if best_index == self.comm.Get_rank():
