...
for i in range(n_nodes/2):
    add_layer.append(Adder(data_base))
    inc_layer.append(Incrementer(data_base))
...
def multilayer_perceptron(x, weights, biases):
    ...
    for i in range(1,n_layer+2):
        ...
        print tf.shape(layer[-1])
        if i%2 == 1:
            for am, a, b in zip(add_layer, layer[-1][0:n_nodes/2], layer[-1][-n_nodes/2:-1]):
                layer.append(am.ex(a, b))
        else:
            for im, a, b in zip(inc_layer, layer[-1][0:n_nodes/2], layer[-1][-n_nodes/2:-1]):
